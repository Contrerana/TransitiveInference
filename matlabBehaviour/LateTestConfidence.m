function T=LateTestConfidence(ID,FilesPath,H,Session)
...
%         errordlg('acc and RT different');
    ...
    if ~isequal(numel(CI),numel(RT))
        errordlg('No_CI and RT different');return;
    end
    % find out the category/condition/premise pair
    temp=IdentifyAllPairsConfidence(ID,Horder,PicsNo,H,Answers,RT,Session,b,CI);
